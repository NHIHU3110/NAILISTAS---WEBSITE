...
AuthUtils.setCurrentUser({ name, email, role: 'user' });
...
// Optionally: lưu profile riêng nếu cần
const profile = JSON.parse(localStorage.getItem(`acct:${user.email}:profile`) || "{}");
localStorage.setItem(`acct:${user.email}:profile`, JSON.stringify({
  email: user.email,
